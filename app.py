...
with tab1:
    ...
    with fig_col3:
        ...
        fig = px.pie(payment_counts, values='Count', names='Payment Type', color_discrete_sequence=px.colors.sequential.RdBu)
        st.plotly_chart(fig, use_container_width=True)
    ...
    with fig_col4:
        ...
        fig = px.bar(passenger_counts, x='Passenger Count', y='Count', color_discrete_sequence=['#636EFA'])
        st.plotly_chart(fig, use_container_width=True)
...
with tab2:
    ...
    fig = px.bar(weekday_counts, x='Weekday', y='Count', color='Weekday', color_discrete_sequence=px.colors.qualitative.Pastel)
    st.plotly_chart(fig, use_container_width=True)
...
with tab3:
    ...
    with fig_col1:
        ...
        fig = px.bar(top_pickups, x='Location ID', y='Count', color='Count', color_continuous_scale='Viridis')
        st.plotly_chart(fig, use_container_width=True)
    ...
    with fig_col2:
        ...
        fig = px.bar(top_dropoffs, x='Location ID', y='Count', color='Count', color_continuous_scale='Plasma')
        st.plotly_chart(fig, use_container_width=True)
...
with tab4:
    ...
    fig = px.bar(avg_fare_components, x='Component', y='Average Amount', color='Component')
    st.plotly_chart(fig, use_container_width=True)
    ...
    x_label = 'Trip Distance (miles)' if fare_rel == 'Fare vs. Distance' else 'Trip Duration (minutes)'
    sample_df = df.sample(min(1000, len(df)))
    fig = px.scatter(sample_df, x=x_col, y='total_amount',
                     labels={x_col: x_label, 'total_amount': 'Total Fare ($)'},
